...
//    val c1     = parser.parse(option,args)
   ...
   // val fulldata    = hiveContext.read.parquet("hdfs://localhost:8020/user/hive/warehouse/cif_finup_lend.db/system_regions/*.parquet")
    //只保留code 和 名字
    val fulldataAsRDD = fulldata.map{
      row =>
        val code =row.getAs[String]("code").toLong
        val name =row.getAs[String]("name")
        (code,name)
    }
    //val metadata   = fulldata.columns.map(StructField(_,DataTypes.StringType))
    //新表列的元数据
    val metadata      = Array("regions_code","regions_rate","ancestor_regions_code","ancestor_regions_rate").map(x=>if(x.contains("code"))StructField(x,DataTypes.StringType) else StructField(x,DataTypes.LongType))
    //过滤出叶子数据
    ...
    info =>
      ...
      //为每个code加上rate
      info=>Row.fromSeq(Seq(info._1.toString,ratejudge(info._1),info._2.toString,ratejudge(info._2)))
    }.cache
    println(kankan.map{
      x =>
       // test
...
//      primeTable.registerTempTable(s"$tempTableName")
    ...
    hiveContext.sql(s"Create table $resultTableName stored as parquet as select a.regions_code,b.name as regions_name,a.regions_rate,a.ancestor_regions_code ,a.ancestor_regions_rate ,c.name as ancestor_regions_name FROM $tempTableName a INNER JOIN $sourceTableName b ON a.regions_code = b.code INNER JOIN $sourceTableName c On a.ancestor_regions_code = c.code")//.write.parquet("hdfs://localhost:8020/temp/fuckee")
